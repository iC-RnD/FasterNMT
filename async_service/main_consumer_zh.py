    ...
    try:
        ...
        if src_lang == 'zh':
            ...
            if src_lang.lower() not in config.LIST_LANGUAGES or tgt_lang.lower() not in config.LIST_TARGETS:
                ...
                st_time = time.time()
                res_tran["context"] = ZH_translator.infer(doc)
                ...
                url_calback = body.get("api_callback", "path callback result")
                print("time translate: ", time.time() - st_time, url_calback)
                if url_calback:
...
if __name__ == '__main__':
    cfg = get_config()
    QUEUE_NAME = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_zh"]["Queue"]
    QUEUE_HOST = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_zh"]["HostName"]
    QUEUE_PASSWORD = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_zh"]["Password"]
    QUEUE_USERNAME = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_zh"]["UserName"]
    QUEUE_VIRTUAL_HOST = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_zh"]["VirtualHost"]
    rab = None
    while True:
        try:
            try:
