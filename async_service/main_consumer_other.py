...
OTHER_translator = inference.many2vi('other')
...
def callback_func(body, properties):
    ...
    try:
        doc = body["text"]
        if src_lang != 'en' and src_lang != 'zh':
            st_time = time.time()
            ...
            if src_lang.lower() not in config.LIST_LANGUAGES or tgt_lang.lower() not in config.LIST_TARGETS:
                ...
                url_calback = body.get("api_callback", "")
                if url_calback:
                ...
                res_tran["context"] = OTHER_translator.infer(doc, src_lang)
...
if __name__ == '__main__':
    cfg = get_config()
    QUEUE_NAME = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_other"]["Queue"]
    QUEUE_HOST = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_other"]["HostName"]
    QUEUE_PASSWORD = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_other"]["Password"]
    QUEUE_USERNAME = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_other"]["UserName"]
    QUEUE_VIRTUAL_HOST = cfg["ConfigManager"]["QueueConfigs"]["queue_translate_other"]["VirtualHost"]
    rab = None
    while True:
        try:
            try:
